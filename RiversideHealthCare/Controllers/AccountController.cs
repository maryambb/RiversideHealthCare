    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            if (ModelState.IsValid && WebSecurity.Login(model.UserName, model.Password, persistCookie: model.RememberMe))
            {
                ...
                // Redirect users to proper page based on roles
                if (returnUrl != null)
                {
                    return RedirectToLocal(returnUrl);
                }
                ...
                {
                    if (Roles.GetRolesForUser(model.UserName)[0] == "patient")
                    {
                        return RedirectToAction("Index", "PatientAccount");
                    }
                    else if (Roles.GetRolesForUser(model.UserName)[0] == "doctor")
                    {
                        return RedirectToAction("Index", "DoctorAccount");
                    }
                    else if (Roles.GetRolesForUser(model.UserName)[0] == "admin")
                    {
